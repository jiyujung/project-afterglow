...
router.get('/list/search/:page',function(req,res,next) {
  ...
  var search = '%' + queryData.bathandbody + '%';
  pool.getConnection(function(err, connection) {
    var sql = "SELECT * FROM product WHERE p_category=2 and (p_brand like ? or p_name like ? or p_note like ?)";
...
router.get('/list/recent/:page',function(req,res,next) {
  var page = req.params.page;
  pool.getConnection(function(err, connection) {
    var sql = "SELECT * FROM product WHERE p_category=2 ORDER BY p_id desc";
...
router.get('/list/high/:page',function(req,res,next) {
  var page = req.params.page;
  pool.getConnection(function(err, connection) {
    var sql = "SELECT * FROM product WHERE p_category=2 ORDER BY p_price desc";
    connection.query(sql, function(err,rows) {
      if(err) console.error(err);
...
router.get('/list/low/:page',function(req,res,next) {
  var page = req.params.page;
  pool.getConnection(function(err, connection) {
    var sql = "SELECT * FROM product WHERE p_category=2 ORDER BY p_price asc";
    connection.query(sql, function(err,rows) {
      if(err) console.error(err);
