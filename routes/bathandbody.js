...
router.get('/list/:page', function(req, res, next) {
  const page = req.params.page;
    pool.getConnection(function(err, connection) {
        const sqlForSelectList = "SELECT p_id, p_name, p_price, p_brand, p_img FROM product WHERE p_category=2";
...
router.get('/list/search/:page',function(req,res,next) {
  ...
  const search = '%' + queryData.bathandbody + '%';
  pool.getConnection(function(err, connection) {
    const sql = "SELECT * FROM product WHERE p_category=2 and (p_brand like ? or p_name like ? or p_note like ?)";
...
router.get('/list/recent/:page',function(req,res,next) {
  const page = req.params.page;
  pool.getConnection(function(err, connection) {
    const sql = "SELECT * FROM product WHERE p_category=2 ORDER BY p_id desc";
...
router.get('/list/high/:page',function(req,res,next) {
  const page = req.params.page;
  pool.getConnection(function(err, connection) {
    const sql = "SELECT * FROM product WHERE p_category=2 ORDER BY p_price desc";
    connection.query(sql, function(err,rows) {
      if(err) console.error(err);
...
router.get('/list/low/:page',function(req,res,next) {
  const page = req.params.page;
  pool.getConnection(function(err, connection) {
    const sql = "SELECT * FROM product WHERE p_category=2 ORDER BY p_price asc";
    connection.query(sql, function(err,rows) {
      if(err) console.error(err);
