var Slider = function(id, _web, _tab, _mobile){
    ...
    for(var i=0; i<items.length; i++){
      ...
      items[i].style.width = '700px';
      items[i].style['margin-right'] = '40px';
    }
    ...
    function resize(){
      ...
      if(display === 1){
        ...
        for(var i=0; i<items.length; i++){
          items[i].style.width = '700px';
          items[i].style['margin-right'] = '40px'; // 간격
        }
      }
    }
    ...
    function setDisplayCount(count) {
      ...
      document.querySelector('#'+ id + ' .slider').style.width = (totalCount+1) * 700 + 40 * (totalCount+1) + 'px';
      var items = document.querySelector('#'+ id + ' .slider').children;
      for(var i=0; i<items.length; i++){
        items[i].style.width = '700px';
      }
    ...
    return {
      ...
      next: function(){
        left -= 700;
        var limit = (-1) * 700 * (totalCount - display);
        if(left < limit){
          left = limit;
        }
        document.querySelector('#'+ id + ' .slider').style.left = left + 'px';
  ...
  var slider = new Slider('slider', 4, 0, 0);